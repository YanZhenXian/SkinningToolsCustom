...
def selectedObjectVertexList(includeObjects=False):
    """
    Maya-native vertex selection query.
    Only returns vertices if user explicitly selected vertices.
    Will NOT convert object/face/edge selection.
    """
    sel = cmds.ls(sl=True, fl=True, l=True)
    if not sel:
        return []

    # 只接受真正的 vertex selection
    vtx = [s for s in sel if '.vtx[' in s]
    if not vtx:
        return []

    if includeObjects:
        return [(s.split('.vtx[')[0], s) for s in vtx]
